...
CREATE TABLE IF NOT EXISTS public.profiles (
  ...
  first_login_welcomed BOOLEAN DEFAULT false,
  created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(),
  updated_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now()
);

-- User roles table (for role management - prevents privilege escalation)
...
-- Disease symptoms for symptom checker
...
CREATE POLICY "Disease symptoms are publicly readable" ON public.disease_symptoms
  FOR SELECT USING (true);

CREATE POLICY "Admins can manage disease symptoms" ON public.disease_symptoms
  FOR ALL USING (has_role(auth.uid(), 'admin'));
...
-- Create hero-slides bucket (public)
INSERT INTO storage.buckets (id, name, public)
...
CREATE POLICY "Hero slides are publicly accessible"
ON storage.objects FOR SELECT
USING (bucket_id = 'hero-slides');

CREATE POLICY "Admins can manage hero slides storage"
ON storage.objects FOR ALL
USING (bucket_id = 'hero-slides' AND has_role(auth.uid(), 'admin'));

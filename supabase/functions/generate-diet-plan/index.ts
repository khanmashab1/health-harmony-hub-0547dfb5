...
function selectMeals(foods: Food[], targetCalories: number, userAllergies: string[]): Food[] {
  // Filter out foods matching user allergies
  ...
  // Sort by how close each food's calories are to target, then pick greedily
  ...
  // If we have nothing, just pick the closest single item
...
serve(async (req) => {
  ...
  try {
    ...
    const { age, gender, height, weight, goal, activityLevel, allergies } = await req.json();
    ...
    // 1. Calculate BMR, TDEE, and target calories
    ...
    const foods = (allFoods || []) as Food[];
    const userAllergies = allergies ? allergies.split(",").map((a: string) => a.trim()).filter(Boolean) : [];
    ...
    const formatItems = (items: Food[]) => items.map(f => ({
      meal: f.name,
      calories: f.calories,
    ...
    const plan = {
      ...
      exercisePlan: generateExercisePlan(goal, activityLevel),
      lifestyleTips: generateLifestyleTips(goal, bmiCategory),
      weeklyGoals: generateWeeklyGoals(goal),
    };

    // 7. Save to database
    const profileData = { age: String(age), gender, height: String(height), weight: String(weight), goal, dietaryPreference: "N/A", activityLevel };
    ...
    return new Response(JSON.stringify({ ...plan, planId: savedPlan?.id || null }), {
...
function generateExercisePlan(goal: string, activityLevel: string) {
  const g = goal.toLowerCase();
...
function generateLifestyleTips(goal: string, bmiCategory: string): string[] {
  const tips = [

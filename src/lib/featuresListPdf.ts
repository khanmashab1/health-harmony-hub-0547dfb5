...
const features = {
  ...
  "3. Doctor Dashboard & Features": [
    ["3.1", "Today's Queue Management", "View and manage today's patient queue with real-time status updates."],
    ...
    ["3.8", "Break Time Management", "Configure break periods (e.g., Lunch, Prayer) excluded from available slots."],
    ...
    ["3.13", "Doctor Settings Panel", "Update bio, qualifications, consultation fee, duration, and max patients per day."],
    ...
    ["3.18", "Doctor Application", "New doctors can apply through a multi-step form with document uploads."],
  ],
  ...
  "6. Appointment & Booking System": [
    ...
    ["6.2", "Doctor Profile View", "Detailed profiles showing qualifications, schedule, reviews, and availability."],
    ["6.3", "Real-Time Slot Availability", "Check available slots based on doctor schedule, breaks, and existing bookings."],
  ...
  "9. Payment & Subscription System": [
    ...
    ["9.2", "Doctor Subscription Plans", "Multiple tiered plans (Basic, Professional, Enterprise) with feature gating."],
    ...
    ["9.6", "Subscription Status Tracking", "Real-time subscription status synced via webhooks."],
  ...
  "10. Email & Notification System": [
    ...
    ["10.9", "Platform Summary Report", "Automated report via pg_cron with subscriber stats, AI analytics, revenue, and appointment data."],
  ...
  "12. SEO & Public Pages": [
    ...
    ["12.7", "SEO Landing Pages", "Dedicated landing pages for key search terms."],
...
export function downloadFeaturesListPDF() {
  const doc = new jsPDF();
  ...
  doc.setFontSize(36);
  doc.text("MediCare+", pageWidth / 2, 90, { align: "center" });

  doc.setFontSize(16);
  doc.text("Smart Healthcare Appointment", pageWidth / 2, 108, { align: "center" });
  doc.text("Management System", pageWidth / 2, 118, { align: "center" });

  doc.setFontSize(22);
  doc.setFont("helvetica", "bold");
  doc.text("Complete Feature List", pageWidth / 2, 145, { align: "center" });

  doc.setFont("helvetica", "normal");
  doc.setFontSize(11);
  doc.text("Final Year Project Documentation", pageWidth / 2, 165, { align: "center" });
  doc.text("90+ Features • 15 Categories • 5 User Roles", pageWidth / 2, 175, { align: "center" });

  doc.setFontSize(10);
  doc.text("Students: Mashab Jadoon, Sardar Zohaib Ahmed, Hashir Iqbal", pageWidth / 2, 215, { align: "center" });
  doc.text("Supervisor: Dr. Kamran Javed", pageWidth / 2, 225, { align: "center" });
  doc.text("Technology: React • TypeScript • Tailwind CSS • Supabase • Stripe", pageWidth / 2, 240, { align: "center" });

  doc.setFontSize(9);
  doc.text("© 2025 MediCare Plus — All Rights Reserved", pageWidth / 2, pageHeight - 20, { align: "center" });
  ...
  doc.setFontSize(20);
  doc.setFont("helvetica", "bold");
  doc.text("Table of Contents", pageWidth / 2, 28, { align: "center" });
  doc.setFontSize(9);
  doc.setFont("helvetica", "normal");
  doc.text("14 Categories • 80+ Features", pageWidth / 2, 38, { align: "center" });

  y = 58;
  const sectionNames = Object.keys(features);
  const featureCounts = Object.values(features).map(f => f.length);
  ...
  sectionNames.forEach((name, i) => {
    ...
    doc.setFontSize(9);
    doc.text(`${featureCounts[i]} features`, pageWidth - 20, y, { align: "right" });
  ...
  doc.setFont("helvetica", "bold");
  doc.setTextColor(15, 118, 110);
  doc.text("Summary Statistics", pageWidth / 2, y + 12, { align: "center" });

  doc.setFontSize(9);
  doc.setFont("helvetica", "normal");
  doc.setTextColor(60);
  const stats = [
    "Total Features: 90+  •  User Roles: 5  •  Edge Functions: 18+",
    "Database Tables: 30+  •  AI Features: 10  •  Email Types: 12",
    "SEO Pages: 10  •  Cron Jobs: 3  •  Feature Categories: 15",
  ];
  stats.forEach((s, i) => {
    doc.text(s, pageWidth / 2, y + 23 + i * 8, { align: "center" });
  });
  ...
  const sectionColors: [number, number, number][] = [
    ...
    [20, 184, 166],   // teal-alt
    [99, 102, 241],   // indigo
    [168, 85, 247],   // purple-alt
    [34, 197, 94],    // green
    [37, 99, 235],    // blue-alt
  ];

  Object.entries(features).forEach(([sectionName, rows], idx) => {
    doc.addPage();
    ...
    doc.setFontSize(16);
    doc.setFont("helvetica", "bold");
    doc.text(sectionName, pageWidth / 2, 16, { align: "center" });
    doc.setFontSize(9);
    doc.setFont("helvetica", "normal");
    doc.text(`${rows.length} Features`, pageWidth / 2, 28, { align: "center" });
  ...
  for (let i = 2; i <= pageCount; i++) {
    ...
    doc.text("MediCare+ — Complete Feature List • Final Year Project 2025", pageWidth / 2, pageHeight - 4, { align: "center" });
  }

  doc.save("MediCare-Plus-Complete-Feature-List.pdf");
}
